    ...
    "select": {
        activateForm: function () {
            ...
            for (var i = 0, length = a.length; i < length; i++) {
                ...
                if (optgroup != null && select_elt.find('optgroup[label="' + optgroup + '"]').length == 0) {
                	var optgroup_elt = jQuery(document.createElement('optgroup')).attr("label", optgroup);
                	var option_elt = jQuery(document.createElement('option'))
                    	.val(key)
                    	.html(value);
                	if (String(key) === String(currentCollectionValue)) option_elt.attr('selected', 'selected');
                	select_elt.append(optgroup_elt.append(option_elt));
                }
                	...
                	var option_elt = jQuery(document.createElement('option'))
                    	...
                    	.html(value);
                	if (String(key) === String(currentCollectionValue)) option_elt.attr('selected', 'selected');
                	select_elt.find('optgroup[label="' + optgroup + '"]').first().append(option_elt);
                }
                else {
	                var option_elt = jQuery(document.createElement('option'))
	                    .val(key)
